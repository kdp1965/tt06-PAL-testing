...
def generate_bitstream(filename, fsm=False, debug=False):
    output = []
    ...
    if fsm:
        ...
        for c_b in c_bytes:
            c_b_adj = c_b[:4] + c_b[-4:]
            bitstream_hex += bin_to_hex[c_b_adj[0:4]] + bin_to_hex[c_b_adj[4:8]]

        output.append("    'prog' : [")
        inline = 0
        bitstream_hex = "000000" + bitstream_hex

        line = "              "
        for i in range(0, len(bitstream_hex), 8):
            idx = 56 - i
            line += f'0x{bitstream_hex[idx:idx+8]}, '
            inline += 1
            if inline == 4 and idx != 0:
                output.append(line)
                line = "              "
                inline = 0
...
#        for i in range(0,len(bitstream_hex),8):
#            idx = 56 - i
#            print(f'0x{bitstream_hex[idx:idx+8]}, ', end='')
#            inline += 1
#            if inline == 4 and idx != 0:
#                print('')
#                print("              ", end='')
...
#
        ...
        # ====================================================================================
        # Generate an RP2040 PIO FIFO array
        # ====================================================================================
        
        print("")
        print("RP2040 PIO array:")
        print("    PROGMEM = [", end='')
        inline=0
        bitstream_hex = "000000" + bitstream_hex
        for i in range(0,len(bitstream_hex),8):
...
if __name__ == "__main__":
    ...
    parser.add_argument('--string', type=str, default="", help='FSM String')
    parser.add_argument('--fsm',  action="store_true", default=False, help='Create FSM type output only')
    ...
    result = generate_bitstream(filename, fsm, debug)
    for l in result:
        print(l)
